...
def simulation(per):
    ...
    del model
    return np.mean(np.array(wait_time))

wait_time_list = []
for per in range(0,101):
    print(per)
    wait_time_list_in = [per/100]
    for repeat in range(10):
        wait_time_list_in.append(simulation(per))
    wait_time_list.append(wait_time_list_in)
    print(wait_time_list_in)
model = RestroomModel()
model.write_csv('./result_5.csv',wait_time_list)